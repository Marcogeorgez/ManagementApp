        ...
        try
        {
            ...
            if (message.IsApproved)
            {
                ...
                foreach (var user in usersToNotify)
                {
                    ...
                    {
                        UserId = user.Id,
                        Subject = "You have a new message on Operavue ⚡",
            ...
            else if (!message.IsApproved)
            {
                ...
                foreach (var user in adminUsers)
                {
                    ...
                    {
                        UserId = user.Id,
                        Subject = "You have a new message approval request on Operavue ⚡",
        ...
        foreach (var admin in adminUsers)
        {
            var notificationItem = new NotificationQueueItem
            {
                ...
                Message = $@"
                            ...
                            <a href='https://Operavue.ManagementApp.net/project' target='_blank'>Operavue</a>.</p>",
                CreatedAt = DateTime.UtcNow
            };
    ...
    public async Task NewUserJoinedNoitifcation(ApplicationUser User)
    {
        try
        {
            ...
            foreach (var admin in adminUsers)
            {
                ...
                {
                    UserId = admin.Id,
                    Subject = $"New User {User.UserName} has requested to join Operavue ⚡",
                    Message = $@"
                            <p>The user <strong>{User.UserName}</strong> with email {User.Email} has joined as a guest on 
                            <a href='https://Operavue.ManagementApp.net/project' target='_blank'>Operavue</a> please review it and assign a role to them.</p>",
                    CreatedAt = DateTime.UtcNow
                };
                await AddToQueue(notificationItem);
            }
        ...
        try
        {
            ...
            foreach (var user in users)
            {
                if (UserId != user)
                {
                    ...
                    {
                        UserId = user,
                        Subject = $"Client {clientUser.UserName} Preferences Updated at Operavue ⚡",
                        Message = $@"
                            ...
                            <a href='https://Operavue.ManagementApp.net/project' target='_blank'>Operavue</a> please review to stay up to date.</p>",
                        CreatedAt = DateTime.UtcNow
                    };
                    await AddToQueue(notificationItem);
                }
            ...
            foreach (var admin in adminUsers.Where(u => u.Id != updatedByUserId))
            {
                ...
                var notificationItem = new NotificationQueueItem
                {
                    ...
                    Message = $@"
                            ...
                            <a href='https://Operavue.ManagementApp.net/project' target='_blank'>Operavue</a>.
                ...
                if (project.ClientId != null && project.ClientId != updatedByUserId)
                {
                    ...
                    if (client != null)
                    {
                        var notificationItem = new NotificationQueueItem
                        {
                            ...
                            Message = $@" <p>The project '<strong>{project.ProjectName}</strong>' has been marked as '<strong>{newStatus}</strong>' on <a href='https://Operavue.ManagementApp.net/project' target='_blank'>Operavue</a>.</p>",
                            CreatedAt = DateTime.UtcNow
                        };
                        await AddToQueue(notificationItem);
                    }
            ...
            if (newStatus == ProjectStatus.Delivered)
            {
                if (project.ClientId != null && project.ClientId != updatedByUserId)
                {
                    ...
                    if (client != null)
                    {
                        ...
                        {
                            UserId = client.Id,
                            Subject = $"You have a new project delivered by Management Company ⚡",
                            Message = $@"
<!DOCTYPE html>
<html lang='en'>
<head>
