...
using ManagementApp.Services.Helpers;
...
using static ManagementApp.Components.Pages.CalenderPage;
    ...
    public async Task UpdateProjectAsync(Project project, string updatedByUserId)
    {
        using (var context = _contextFactory.CreateDbContext())
        {
            ...
            if (_project != null)
            {
                ...
                var description = _project.Description;
                var musicPreference = _project.MusicPreference;
                context.Entry(_project).CurrentValues.SetValues(project);
                _project.Description ??= project.Description == null ? description : null;
                _project.MusicPreference ??= project.MusicPreference == null ? musicPreference : null;
    ...
    public async Task UpdateProjectsInBatchAsync(IEnumerable<Project> projects, string updatedByUserId)
    {
        using (var context = _contextFactory.CreateDbContext())
        {
            ...
            foreach (var project in projects)
            {
                ...
                var description = dbProject.Description;
                var musicPreference = dbProject.MusicPreference;
                // Update project values
                context.Entry(dbProject).CurrentValues.SetValues(project);

                dbProject.Description ??= project.Description == null ? description : null;
                dbProject.MusicPreference ??= project.MusicPreference == null ? musicPreference : null;
