...
public partial class ProjectPage : Microsoft.AspNetCore.Components.ComponentBase
{
    ...
    private async Task OpenActionsDialog(Project item)
    {
        var parameters = new DialogParameters
        {
            ["Item"] = item,
        ...
        var dialog = await DialogService.ShowAsync<ProjectActionsDialog>("Project Actions", parameters, options);
        var result = await dialog.Result;
        ...
        if (result != null && !result.Canceled && result.Data is string action)
        {
            switch (action)
            {
                case "Edit":
                    ...
                    break;
                case "Archive":
                    await ArchiveProject(item);
                    break;
                case "Unarchive":
                    await UnarchiveProject(item);
                    break;
                case "Delete":
                    await DeleteDialog(item);
                    break;
                case "Logging":
                    await Logging(item, _currentUserId);
                    break;
                case "CopyName":
                    await CopyProjectNameToClipboard(item);
                    break;
                case "Duplicate":
